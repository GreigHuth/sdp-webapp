...
def get():
    ...
    if request.method == 'POST':  
        ...
        #escapes non-aplphanumeric characters to prevent SQL injections
        query = re.escape(form.search.data)
        return redirect(url_for('get_search', query=query))
   
    return render_template('get.html', title = 'Get Book', form=form)


@app.route('/get/search/<query>', methods=['GET', 'POST'])
@login_required
def get_search(query):
    flash('Searched for %s' % str(query))
    #search the database for book titles 
    return render_template('get_search.html', query=query)
